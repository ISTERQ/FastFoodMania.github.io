...
document.getElementById('registrationForm').addEventListener('submit', function(event) {
    event.preventDefault(); // Предотвращаем стандартное поведение HTML формы
    ...
    .then(data => {
        alert(data); // Отображаем сообщение от сервера
        if (data.includes('Регистрация успешна')) {
            closeModal(document.getElementById('loginModal')); // Закрытие модального окна
        }
    })
    .catch(error => {
